...
export const cloudinaryAuth = PieceAuth.CustomAuth({
    ...
    props: {
        api_key: Property.ShortText({
            displayName: 'API Key',
            description: 'Your Cloudinary API Key. Found in Console Settings â†’ API Keys page.',
            required: true,
        }),
        api_secret: PieceAuth.SecretText({
            displayName: 'API Secret',
            description: 'Your Cloudinary API Secret. Found in Console Settings â†’ API Keys page. Keep this secure and never share publicly.',
            required: true,
        }),
        cloud_name: Property.ShortText({
            displayName: 'Cloud Name',
            description: 'Your Cloudinary Cloud Name. Found in Console Settings â†’ API Keys page or in your dashboard URL (e.g., "demo" from console.cloudinary.com/console/demo).',
            required: true,
        }),
    },
    validate: async ({ auth }) => {
        try {
            ...
            return {
                valid: false,
                error: 'Invalid credentials. Please check your API Key, API Secret, and Cloud Name in your Cloudinary Console Settings â†’ API Keys page.',
            };
        }
    },
});
