...
- Two Projects in IOPeer: one for Development and one for Production
...
<Tip>
Don't forget to replace `INSTANCE_URL` and `PROJECT_ID`, and add `IOPEER_API_KEY` to the secrets.
</Tip>
      ...
      - name: deploy
        ...
        run: |
          curl --request POST \
            ...
            --header 'Authorization: Bearer ${{ secrets.IOPEER_API_KEY }}' \
