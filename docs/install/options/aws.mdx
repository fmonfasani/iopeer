---
title: "AWS (Pulumi)"
description: "Get IOPeer up & running on AWS with Pulumi for IaC"
---
...
If you're already familiar with Pulumi Cloud and have [integrated their services with your AWS account](https://www.pulumi.com/docs/pulumi-cloud/deployments/oidc/aws/#configuring-openid-connect-for-aws), you can use the button below to deploy IOPeer in a few clicks. 
The template will deploy the latest IOPeer image that's available on [Docker Hub](https://hub.docker.com/r/iopeer/iopeer).

[![Deploy with Pulumi](https://get.pulumi.com/new/button.svg)](https://app.pulumi.com/new?template=https://github.com/iopeer/iopeer/tree/main/deploy/pulumi)

## Deploy from a local environment

Or, if you're currently using an S3 bucket to maintain your Pulumi state, you can scaffold and deploy IOPeer direct from Docker Hub using the template below in just few commands:

```bash
$ mkdir deploy-iopeer && cd deploy-iopeer 
$ pulumi new https://github.com/iopeer/iopeer/tree/main/deploy/pulumi
$ pulumi up
```
...
Opting out of this will mean relying on using the application load balancer's url over standard HTTP to access your IOPeer deployment.

For a full list of all the currently available configuration options, take a look at the [IOPeer Pulumi template file on GitHub](https://github.com/iopeer/iopeer/tree/main/deploy/pulumi/Pulumi.yaml).

## Setting up Pulumi for the first time

If you're new to Pulumi then read on to get your local dev environment setup to be able to deploy IOPeer.
...
4. Create an S3 bucket which we'll use to maintain the state of all the various service we'll provision for our IOPeer deployment:
...
```
6. Next we're going to use the IOPeer Pulumi deploy template to create a new project, a stack in that project and then kick off the deploy:
...
<Tip>
    Note: When choosing a stack name, use something descriptive like `iopeer-dev`, `ap-prod` etc. 
...
```json
    _: {
        IOpeerUrl: "http://<alb name & id>.us-east-1.elb.amazonaws.com"
        iopeerEnv: [
         . . . .
        ]
       }
...
The config value of interest here is the `IOpeerUrl` as that is the URL for our IOPeer deployment. 
...
Congratulations! You have successfully deployed IOPeer to AWS.

## Deploy a locally built IOPeer Docker image

To deploy a locally built image instead of using the official Docker Hub image, read on.

1. Clone the IOPeer repo locally:

```bash
git clone https://github.com/iopeer/iopeer
```
...
```
3. This folder already has two Pulumi stack configuration files reday to go: `Pulumi.iopeer-dev.yaml` and `Pulumi.iopeer-prod.yaml`. 
...
Lets continue by creating a development stack that uses the existing `Pulumi.iopeer-dev.yaml` file & kick off the deploy.

```bash
pulumi stack init iopeer-dev && pulumi up
```

<Tip>
    Note: Using `iopeer-dev` or `iopeer-prod` for the `pulumi stack init` command is required here as the stack name needs to match the existing stack file name in the folder. 
</Tip>
...
Congratulations! You have successfully deployed IOPeer into AWS using a locally built Docker image.
...
Or maybe you'd still like to deploy the official IOPeer Docker image instead of a local build, but would like to change some of the services. Simply set the `deployLocalBuild` config option in the stack file to `false` and make whatever changes you'd like to the `index.ts` file.   
