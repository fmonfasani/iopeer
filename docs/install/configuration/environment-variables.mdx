...
To configure **IOPeer**, you will need to set the required environment variables. There is file called `.env` at the root directory for our main repo.

<Tip> When you execute the [tools/deploy.sh](https://github.com/iopeer/iopeer/blob/main/tools/deploy.sh) script in the Docker installation tutorial,
it will produce these values. </Tip>

<Info>
  IOPeer currently keeps the historical `AP_` environment variable prefix (and defaults like `~/.IOpeer`) to preserve
  compatibility with existing deployments. Examples in this guide may show branded paths such as `~/.iopeer`, but the default
  value remains `~/.IOpeer` unless you override it. Any future transition to an `IOP_` prefix or new default paths will
...
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| `AP_CONFIG_PATH`                   | Optional parameter for specifying the path to store SQLite3 and local settings.                                                                                                                                      | `~/.IOpeer`                                                              |                                                                        |
| `AP_CLOUD_AUTH_ENABLED`            | Turn off the utilization of IOPeer oauth2 applications                                                                                                                                                         | `false`                                                                        |                                                                        |
| `AP_DB_TYPE`                       | The type of database to use. (POSTGRES / SQLITE3)                                                                                                                                                                    | `SQLITE3`                                                                      |                                                                        |
| `AP_EXECUTION_MODE`                | You can choose between 'SANDBOXED', 'UNSANDBOXED', 'SANDBOX_CODE_ONLY' as possible values. If you decide to change this, make sure to carefully read https://www.iopeer.com/docs/install/architecture/workers                                                                | `UNSANDBOXED`                                                                  |                                                                        |
...
| `AP_ENCRYPTION_KEY`                | â—ï¸ Encryption key used for connections is a 32-character (16 bytes) hexadecimal key. You can generate one using the following command: `openssl rand -hex 16`.                                                                                                                                                                              | `None`                                                                         |
| `AP_EXECUTION_DATA_RETENTION_DAYS` | The number of days to retain execution data, logs and events.                                                                                                                                                        | `30`                                                                         |                                                                        |
| `AP_FRONTEND_URL`                  | â—ï¸ Url that will be used to specify redirect url and webhook url. 
| `AP_INTERNAL_URL`                  | (BETA) Used to specify the SSO authentication URL.                                                                                 | `None`                                                                         | [https://demo.iopeer.com/api](https://demo.iopeer.com/api) |
| `AP_JWT_SECRET`                    | â—ï¸ Encryption key used for generating JWT tokens is a 32-character hexadecimal key. You can generate one using the following command: `openssl rand -hex 32`.                                                                                                                                                                     | `None`                                                                         | [https://demo.iopeer.com](https://demo.iopeer.com)         |
...
| `AP_POSTGRES_DATABASE`             | â—ï¸ The name of the PostgreSQL database                                                                                                                                                                              | `None`                                                                         |                                                                        |
| `AP_POSTGRES_HOST`                 | â—ï¸ The hostname or IP address of the PostgreSQL server                                                                                                                                                              | `None`                                                                         |                                                                        |
| `AP_POSTGRES_PASSWORD`             | â—ï¸ The password for the PostgreSQL,  you can generate a 32-character hexadecimal key using the following command: `openssl rand -hex 32`.                                                                                                                                                                              | `None`                                                                         |                                                                        |
| `AP_POSTGRES_PORT`                 | â—ï¸ The port number for the PostgreSQL server                                                                                                                                                                        | `None`                                                                         |                                                                        |
| `AP_POSTGRES_USERNAME`             | â—ï¸ The username for the PostgreSQL user                                                                                                                                                                             | `None`                                                                         |                                                                        |
...
| `AP_REDIS_USER`                    | â—ï¸ Username to use when connect to redis                                                                                                                                                                            | `None`                                                                         |                                                                        |
| `AP_REDIS_PASSWORD`                | â—ï¸ Password to use when connect to redis                                                                                                                                                                            | `None`                                                                         |                                                                        |
| `AP_REDIS_HOST`                    | â—ï¸ The hostname or IP address of the Redis server                                                                                                                                                                   | `None`                                                                         |                                                                        |
| `AP_REDIS_PORT`                    | â—ï¸ The port number for the Redis server                                                                                                                                                                             | `None`                                                                         |                                                                        |
...
| `AP_TEMPLATES_SOURCE_URL`          | This is the endpoint we query for templates, remove it and templates will be removed from UI                                                                                                                         | `https://cloud.iopeer.com/api/v1/flow-templates`                         |                                                                        |
...
| `AP_APP_TITLE` | Initial title shown in the browser tab while loading the app | `IOPeer`  | `IOPeer`
| `AP_FAVICON_URL` | Initial favicon shown in the browser tab while loading the app | `https://cdn.iopeer.com/brand/favicon.ico` | `https://cdn.iopeer.com/brand/favicon.ico`

### OpenTelemetry Configuration

IOPeer supports both standard OpenTelemetry environment variables and vendor-specific configuration for observability and tracing.
