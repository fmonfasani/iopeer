...
<Info>
  IOPeer currently keeps the historical `AP_` environment variable prefix (and defaults like `~/.activepieces`) to preserve
  compatibility with existing deployments. Examples in this guide may show branded paths such as `~/.iopeer`, but the default
  value remains `~/.activepieces` unless you override it. Any future transition to an `IOP_` prefix or new default paths will
...
| `AP_JWT_SECRET`                    | ❗️ Encryption key used for generating JWT tokens is a 32-character hexadecimal key. You can generate one using the following command: `openssl rand -hex 32`.                                                                                                                                                                     | `None`                                                                         | [https://demo.iopeer.com](https://demo.iopeer.com)         |
