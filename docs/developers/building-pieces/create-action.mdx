...
export const getIcecreamFlavor = createAction({
  ...
  async run(context) {
    const res = await httpClient.sendRequest<string[]>({
      ...
      url: 'https://cloud.iopeer.com/api/v1/webhooks/RGjv57ex3RAHOgs0YK6Ja/sync',
      headers: {
...
To make the action readable by IOPeer, add it to the array of actions in the piece definition.
...
To test the action, use the flow builder in IOPeer. It should function as shown in the screenshot.
