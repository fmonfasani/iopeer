...
> = {
  ...
  items: async ({ auth, propsValue, lastFetchEpochMS }) => {
    const request: HttpRequest = {
      ...
      url: 'https://cloud.iopeer.com/api/v1/webhooks/aHlEaNLc6vcF1nY2XJ2ed/sync',
      headers: {
        authorization: auth,
      },
    };
...
To make the trigger readable by IOPeer, add it to the array of triggers in the piece definition.
...
export const gelato = createPiece({
  ...
  logoUrl: 'https://cdn.iopeer.com/pieces/gelato.png',
  authors: [],
  auth: gelatoAuth,
...
To test the trigger, use the load sample data from flow builder in IOPeer. It should function as shown in the screenshot.
