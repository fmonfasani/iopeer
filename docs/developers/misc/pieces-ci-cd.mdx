...
You can use the IOPeer CLI to sync custom pieces. There is no need to rebuild the Docker image as they are loaded directly from npm.

### How It Works

Use the IOPeer CLI to sync items from `packages/pieces/custom/` to instances. In production, IOPeer acts as an npm registry, storing all piece versions.
...
To use the IOPeer CLI, follow these steps:
