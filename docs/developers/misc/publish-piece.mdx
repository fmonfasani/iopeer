...
You can use the IOPeer CLI to publish custom pieces to the platform. This process packages the pieces and uploads them to the specified API endpoint.

### How It Works

The IOPeer CLI scans the `packages/pieces/` directory for the specified piece. It checks the **name** and **version** in the `package.json` file. If the piece is not already published, it builds, packages, and uploads it to the platform using the API.
...
2. Install the IOPeer CLI by cloning the repository.
...
4. You will be asked three questions to publish your piece:
  ...
  - `API URL`: This is the URL of the API endpoint where the piece will be published (ex: https://cloud.iopeer.com/api).
...
? Enter the API URL : https://cloud.iopeer.com/api
