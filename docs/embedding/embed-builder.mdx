...
This documentation explains how to embed the IOPeer iframe inside your application and customize it.

## Configure SDK

Adding the embedding SDK script will initialize an object in your window called `iopeer`, which has a method called `configure` that you should call after the container has been rendered.
<Tip>
...
```html
<script src="https://cdn.iopeer.com/sdk/embed/0.4.0.js">
</script>
<script>
...
| --- | --- | --- | --- |
| instanceUrl| ✅ |  string | The url of the instance hosting IOPeer, could be https://cloud.iopeer.com if you are a cloud user. |
| jwtToken | ✅  |  string | The jwt token you generated to authenticate your users to IOPeer. |
| prefix | ❌ | string | Some customers have an embedding prefix, like this `<embedding_url_prefix>/<IOPeer_url>`. For example if the prefix is `/automation` and the IOPeer url is `/flows` the full url would be `/automation/flows`. |
| embedding.containerId | ❌ | string | The html element's id that is going to be containing IOPeer's iframe. |
...
| navigation.handler  | ❌ | `({route:string}) => void`  | This callback will be triggered each time a route in IOPeer changes, you can read more about it [here](/embedding/navigation) |
<Tip>
